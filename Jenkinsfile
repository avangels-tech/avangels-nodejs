pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git url: 'https://github.com/avangels-tech/avangels-nodejs.git', branch: 'main'
            }
        }
        stage('Run Tests') {
            steps {
        ...
        stage('Build and Push Docker Image') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD')]) {
                    container('docker') {
                        sh 'docker build -t avangelstech/avangels-nodejs:${BUILD_NUMBER} .'
                        sh 'echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin'
                        sh 'docker push avangelstech/avangels-nodejs:${BUILD_NUMBER}'
                    }
                }
            }
        }
